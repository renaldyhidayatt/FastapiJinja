...
from fastapi import FastAPI, Query, Path, HTTPException, status, staticfiles, Body, Form, Request
...
def edit_car(request: Request, id: int = Query(...)):
    ...
    if not car:
        return templates.TemplateResponse(
            "search.html",
            {"request": request, "id": id, "car": car, "title": "Edit Car"},
            status_code=status.HTTP_404_NOT_FOUND,
        )
    return templates.TemplateResponse(
        "edit.html", {"request": request, "id": id, "car": car, "title": "Edit Car"}
    )
